 ...
 * Student Dashboard
 */
$pageTitle = 'ระบบบริหารจัดการสำหรับนักศึกษา';
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM repair WHERE user_id = ? AND status = 'pending'");
$stmt->execute([$userId]);
...
<div class="container-fluid px-0">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3 mb-1 text-gray-800"><i class="bi bi-speedometer2 me-2 text-primary"></i>แดชบอร์ดนักศึกษา</h1>
            ...
            <div class="card bg-white border-0 shadow-sm h-100 py-2 border-start border-4 border-primary">
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            <div class="text-xs font-weight-bold text-primary text-uppercase mb-1">รายการแจ้งซ่อมทั้งหมด
                            </div>
                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?= number_format($totalRepairs) ?>
                                รายการ</div>
                        </div>
                        <div class="col-auto">
                ...
                <div class="card-body">
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800"><?= number_format($pendingCount) ?>
                                รายการ</div>
                        </div>
                        <div class="col-auto">
                                        ...
                                        <tr>
                                            <td class="ps-4">
                                                <div class="d-flex flex-column">
                                                    <span
                                                        class="fw-medium text-dark"><?= htmlspecialchars($r['item_name']) ?></span>
                                                    <span class="small text-muted"><?= htmlspecialchars($r['code']) ?></span>
                                                </div>
                                            </td>
                                            <td><span
                                                    class="text-muted"><?= mb_substr(htmlspecialchars($r['issue']), 0, 40) ?>...</span>
                                            </td>
                                            <td>
                                            ...
                                            <td class="hide-mobile pe-4 text-end text-muted small">
                                                <?= formatDateThai($r['created_at']) ?></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm mb-4 bg-primary text-white overflow-hidden position-relative">
                <div class="card-body text-center p-4 position-relative" style="z-index: 1;">
                    ...
                    <p class="mb-4 text-white-50">แจ้งซ่อมได้ทันที ติดตามสถานะได้ตลอดเวลา พร้อมแจ้งเตือนเมื่อซ่อมเสร็จ
                    </p>
                ...
                <!-- Decorative Icon -->
                <i class="bi bi-laptop position-absolute"
                    style="font-size: 8rem; opacity: 0.1; right: -20px; bottom: -20px; color: white;"></i>
            </div>
        </div>
    </div>
</div>
