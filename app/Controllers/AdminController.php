...
class AdminController extends Controller
{
    ...
    /**
     * ดาวน์โหลดไฟล์อัปโหลดเป็น zip (equipment + repairs)
     * ใช้สำหรับ action=files
     */
    private function downloadFilesBackup()
    {
        $uploadPath = defined('UPLOAD_PATH') ? UPLOAD_PATH : __DIR__ . '/../../uploads/';
        $uploadPath = rtrim($uploadPath, '/\\') . DIRECTORY_SEPARATOR;

        if (!is_dir($uploadPath)) {
            // ถ้าไม่มีโฟลเดอร์ ให้ redirect พร้อม flash
            if (function_exists('setFlash')) {
                setFlash('danger', 'ไม่พบโฟลเดอร์ uploads');
            }
            $this->redirect(SITE_URL . '/backup');
        }

        if (!class_exists('ZipArchive')) {
            if (function_exists('setFlash')) {
                setFlash('danger', 'เซิร์ฟเวอร์ไม่รองรับ ZipArchive (php-zip) กรุณาติดตั้ง extension zip');
            }
            $this->redirect(SITE_URL . '/backup');
        }

        try {
            logActivity(getCurrentUserId(), 'Backup', 'ดาวน์โหลดไฟล์อัปโหลด (zip)');
        } catch (Exception $e) {
        }

        $filename = 'uploads_' . date('Y-m-d_His') . '.zip';
        ...
        // สร้างไฟล์ชั่วคราว
        $tmpBase = tempnam(sys_get_temp_dir(), 'uploads_');
        if ($tmpBase === false) {
            $tmpBase = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'uploads_' . uniqid();
        }
        // tempnam สร้างไฟล์เปล่า — ลบแล้วเติม .zip
        ...
        if ($res !== true) {
            if (function_exists('setFlash')) {
                setFlash('danger', 'ไม่สามารถสร้างไฟล์ zip ได้ (code: ' . $res . ')');
            }
            $this->redirect(SITE_URL . '/backup');
        }

        $folders = ['equipment', 'repairs'];
        $added = 0;
        $uploadReal = realpath($uploadPath);

        foreach ($folders as $folder) {
            $full = $uploadPath . $folder;
            if (!is_dir($full)) {
                continue;
            }
            ...
            $iterator = new RecursiveIteratorIterator(
                new RecursiveDirectoryIterator($fullReal, RecursiveDirectoryIterator::SKIP_DOTS),
            ...
            foreach ($iterator as $file) {
                ...
                // ป้องกัน path traversal — ต้องอยู่ใต้ uploads
                if ($uploadReal !== false && strpos($realPath, $uploadReal) !== 0) {
                    continue;
                }
                // คำนวณ path ภายใน zip เช่น equipment/abc.jpg, repairs/xyz.png
                $relative = $folder . '/' . substr($realPath, strlen($fullReal) + 1);
                $relative = str_replace(DIRECTORY_SEPARATOR, '/', $relative);
                // ข้ามไฟล์ที่ไม่ใช่รูปถ้าต้องการ — แต่เก็บทั้งหมดตามสเปค
        ...
        if ($added === 0) {
            $zip->addFromString('README.txt', 'No uploaded files found on ' . date('Y-m-d H:i:s') . "\nFolders checked: equipment, repairs\n");
        }
        ...
        if (!file_exists($tmpFile)) {
            if (function_exists('setFlash')) {
                setFlash('danger', 'สร้างไฟล์ zip ไม่สำเร็จ');
            }
            $this->redirect(SITE_URL . '/backup');
        }
