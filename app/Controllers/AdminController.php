...
class AdminController extends Controller
{
    ...
    private function downloadBackup()
    {
        ...
        try {
            logActivity(getCurrentUserId(), 'สำรองข้อมูล', 'ดาวน์โหลดสำรองข้อมูล');
        } catch (Exception $e) {}
        ...
        $header = "-- ระบบบริหารจัดการครุภัณฑ์และงานซ่อมบำรุง สำรองข้อมูล\n";
        $header .= "-- Database: " . DB_NAME . "\n";
        $header .= "-- วันที่สำรองข้อมูล (พ.ศ.): " . formatDateTimeThai(date('Y-m-d H:i:s')) . "\n";
        $header .= "-- Host: " . DB_HOST . "\n";
        $header .= "-- สร้างโดยระบบสำรองข้อมูล\n\n";
        ...
        $footer .= "-- สำรองข้อมูลเสร็จสิ้น: " . formatDateTimeThai(date('Y-m-d H:i:s')) . "\n";
        $streamWrite($footer);
        exit;
    }
    ...
    private function tryMysqldump($tables)
    {
        ...
        // Log ก่อน passthru
        try {
            logActivity(getCurrentUserId(), 'สำรองข้อมูล', 'ดาวน์โหลดสำรองข้อมูล (mysqldump)');
        } catch (Exception $e) {
        }
    ...
    public function export($type = null)
    {
        ...
        logActivity(getCurrentUserId(), 'ส่งออกข้อมูล', 'ส่งออกข้อมูล: ' . $type);
    ...
    private function exportEquipment()
    {
        ...
        fputcsv($output, ['รหัส', 'รายการ', 'ยี่ห้อ', 'รุ่น', 'ชุด', 'ปี (พ.ศ.)', 'สาขา', 'ห้อง', 'ผู้ถือครอง', 'สถานะ', 'วันที่ซื้อ', 'ราคา', 'วันที่ตรวจ', 'หมายเหตุ']);
        ...
        foreach ($data as $row) {
            fputcsv($output, [
                ...
                formatDateThai($row['purchase_date']), $row['price'], formatDateThai($row['check_date']), $row['remark'],
            ]);
        }
    ...
    private function exportRepairs()
    {
        ...
        $output = fopen('php://output', 'w');
        fputcsv($output, ['ลำดับ', 'ครุภัณฑ์', 'รายการ', 'ปัญหา', 'สถานะ', 'วันที่', 'ผู้แจ้ง']);

        foreach ($data as $row) {
            fputcsv($output, [
                $row['id'], $row['eq_code'], $row['item_name'],
                $row['issue'], translateRepairStatus($row['status']),
                formatDateTimeThai($row['created_at']), $row['firstname'] . ' ' . $row['lastname'],
            ]);
        }
    ...
    private function exportUsers()
    {
        ...
        foreach ($data as $row) {
            fputcsv($output, [
                ...
                formatDateTimeThai($row['created_at']),
            ]);
        }
