<?php
$pageTitle = $pageTitle ?? 'รายละเอียดข้อมูลครุภัณฑ์';
$role = getCurrentRole();
$canEdit = in_array($role, ['admin', 'staff'], true);
$listLabel = $role === 'teacher' ? 'ตรวจสอบครุภัณฑ์' : 'ครุภัณฑ์';
?>

<!-- Page Header -->
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h1><i class="bi bi-pc-display me-2"></i><?= sanitize($pageTitle) ?></h1>
    ...
    <div>
        <?php if ($canEdit): ?>
            <a href="<?= SITE_URL ?>/equipment/edit/<?= (int) $equipment['id'] ?>" class="btn btn-primary">
    ...
    <div class="col-lg-8">
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <table class="table table-borderless mb-0">
                            <tr>
                                ...
                                <td><strong><?= sanitize($equipment['code'] ?? '-') ?></strong></td>
                            </tr>
                            <tr>
                                <th>ชื่อรายการ:</th>
                            ...
                            <tr>
                                <th>ยี่ห้อ/รุ่น:</th>
                                <td><?= sanitize($equipment['brand'] ?? '-') ?> /
                                    <?= sanitize($equipment['model'] ?? '-') ?></td>
                            </tr>
                            <tr>
                                <th>ชุดครุภัณฑ์:</th>
                                <td><?= sanitize($equipment['set_name'] ?? '-') ?>
                                    <?= !empty($equipment['year']) ? '(' . sanitize($equipment['year']) . ')' : '' ?></td>
                            </tr>
                            <tr>
                                <th>สาขา:</th>
                    ...
                    <div class="col-md-6">
                        <table class="table table-borderless mb-0">
                            <tr>
                                ...
                                <td><?= sanitize($equipment['room_name'] ?? '-') ?></td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if (!empty($equipment['holder_firstname'])): ?>
                                        <?= sanitize($equipment['holder_firstname'] . ' ' . $equipment['holder_lastname']) ?>
                                    <?php else: ?>
                ...
                <?php if (!empty($equipment['remark'])): ?>
                    ...
                    <div>
                        ...
                        <?= nl2br(sanitize($equipment['remark'])) ?>
                    </div>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-header">
                <i class="bi bi-wrench me-2"></i>ประวัติการแจ้งซ่อมและการบำรุงรักษา (<?= count($repairHistory) ?> รายการ)
            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td class="hide-mobile">
                                            <?php if (!empty($repair['firstname'])): ?>
                                                <?= sanitize($repair['firstname'] . ' ' . $repair['lastname']) ?>
                                            <?php else: ?>
                                                <span class="text-muted">ผู้ใช้ถูกลบออกจากระบบ</span>
                                            <?php endif; ?>
                                        </td>
                                        <td class="hide-mobile"><?= mb_substr(sanitize($repair['issue']), 0, 50) ?>...</td>
                                        <td>
                    ...
                    <?php if (!empty($purchaseImages)): ?>
                        ...
                        <div class="image-gallery mb-3">
                            <?php foreach ($purchaseImages as $img): ?>
                                <img src="<?= SITE_URL ?>/uploads/<?= sanitize($img['path']) ?>" alt="Purchase Image"
                                    class="lightbox-trigger"
                                    data-img-src="<?= SITE_URL ?>/uploads/<?= sanitize($img['path']) ?>"
                    ...
                    <?php if (!empty($currentImages)): ?>
                        ...
                        <div class="image-gallery">
                            <?php foreach ($currentImages as $img): ?>
                                <img src="<?= SITE_URL ?>/uploads/<?= sanitize($img['path']) ?>" alt="Current Image"
                                    class="lightbox-trigger"
                                    data-img-src="<?= SITE_URL ?>/uploads/<?= sanitize($img['path']) ?>"
