...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$equipment_id)
        $errors[] = 'กรุณาเลือกครุภัณฑ์';
    if (empty($issue))
        $errors[] = 'กรุณากรอกอาการเสีย';

    if (empty($errors)) {
        $stmt = $pdo->prepare("INSERT INTO repair (equipment_id, user_id, issue, status) VALUES (?, ?, ?, 'pending')");
        $stmt->execute([$equipment_id, getCurrentUserId(), $issue]);
        ...
        if (!empty($_FILES['images']['name'][0])) {
            foreach ($_FILES['images']['tmp_name'] as $key => $tmpName) {
                if ($_FILES['images']['error'][$key] === UPLOAD_ERR_OK) {
                    $file = ['name' => $_FILES['images']['name'][$key], 'type' => $_FILES['images']['type'][$key], 'tmp_name' => $tmpName, 'size' => $_FILES['images']['size'][$key]];
                    $result = uploadImage($file, 'repairs');
                    if ($result['success']) {
                        $pdo->prepare("INSERT INTO repair_img (repair_id, path) VALUES (?, ?)")->execute([$repairId, $result['path']]);
                    }
                }
            }
        }

        logActivity($pdo, getCurrentUserId(), 'Submit Repair', "แจ้งซ่อม ID: $repairId");
        setFlash('success', 'ส่งคำร้องแจ้งซ่อมสำเร็จ');
        redirect('my_repairs.php');
    }
}

// Get equipment list
$equipment = $pdo->query("SELECT e.id, e.code, rm.name as room, i.name, i.brand FROM equipment e 
    LEFT JOIN rooms rm ON e.room_id = rm.id
    JOIN items i ON e.item_id = i.id WHERE e.status IN ('available', 'broken') ORDER BY e.code")->fetchAll();
?>
        ...
        <div class="card">
            ...
            <div class="card-body">
                <ul class="mb-0">
                    ...
                    <li>ติดตามสถานะได้ที่หน้า "ประวัติการส่งรายการแจ้งซ่อมบำรุง"</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        imageInput.addEventListener('change', function () {
            ...
            files.forEach(function (file) {
                ...
                // Copy file to hidden input using DataTransfer
